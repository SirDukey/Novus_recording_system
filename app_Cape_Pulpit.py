...
def main_Cape_Pulpit():

    name = 'Cape_Pulpit'
    url = 'http://albert.antfarm.co.za:8000/RadioPulpit'

    if request.method == 'POST':

        if 'Cape_Pulpit_start' in request.form:

            print('Cape_Pulpit_start')
            record(name, url)

        elif 'Cape_Pulpit_stop' in request.form:

            print('Cape_Pulpit_stop')
            try:
                with open('pids/Cape_Pulpit.pid', 'r') as f:
                    try:
                        kill_pid(f.read(), name, url)
                    except:
                        print('no such process')
            finally:
                with open('pids/Cape_Pulpit.pid', 'w') as f:
                    f.write('none')
