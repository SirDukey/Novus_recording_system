...
def main_99FM():
    ...
    name = '99FM'
    url = 'http://ice31.securenetsystems.net/99FM?type=mp3'

    if request.method == 'POST':

        if '99FM_start' in request.form:

            print('99FM_start')
            record(name, url)

        elif '99FM_stop' in request.form:

            print('99FM_stop')
            try:
                with open('pids/99FM.pid', 'r') as f:
                    try:
                        kill_pid(f.read(), name, url)
                    except:
                        print('no such process')
            finally:
                with open('pids/99FM.pid', 'w') as f:
                    f.write('none')
