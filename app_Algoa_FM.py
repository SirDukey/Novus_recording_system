...
def main_Algoa_FM():
    ...
    name = 'Algoa_FM'
    url = 'https://edge.iono.fm/xice/54_medium.aac?p=embed'

    if request.method == 'POST':

        if 'Algoa_FM_start' in request.form:

            print('Algoa_FM_start')
            record(name, url)

        elif 'Algoa_FM_stop' in request.form:

            print('Algoa_FM_stop')
            try:
                with open('pids/Algoa_FM.pid', 'r') as f:
                    try:
                        kill_pid(f.read(), name, url)
                    except:
                        print('no such process')
            finally:
                with open('pids/Algoa_FM.pid', 'w') as f:
                    f.write('none')
